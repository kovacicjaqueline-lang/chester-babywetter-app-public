...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .trip-sheet{width:100%;max-width:680px;min-width:0;max-height:min(92dvh,860px);overflow:auto;scroll-behavior:auto}.trip-sheet-header{position:sticky;top:0;z-index:3;background:rgba(255,250,246,.96);backdrop-filter:blur(10px)}.trip-sheet-header .eyebrow{margin:0 0 2px}.trip-sheet-header h2{margin:0}
...
function actionText(action, assetStore) {
  if (action.kind === 'safety_instruction') return NOTICE_COPY[action.noticeCode]?.[0] ?? action.noticeCode ?? 'Sicherheitshinweis beachten';
...
function transitionRow(transition, snapshot, assetStore, styleTheme, index) {
  ...
  appendActionGroup(groups, 'Sicherheit', actions.filter((action) => action.kind === 'safety_instruction'), assetStore);
